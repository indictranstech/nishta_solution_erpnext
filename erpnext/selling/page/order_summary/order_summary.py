...
def print_order(sel_date=None):
	...
	items = {}
	send_data = []
	order_ids = frappe.db.sql("select name,notw,customer_delivery_time,deliver_mode,pickup_store,door_delivery_sale_point,company  from `tabSales Order` where type_of_order!='Branch Transfer' and status!='Cancelled' and date(`deliver_date`)=%s ", (add_date), as_dict=True);
	for order_ids_val in order_ids:
		...
		basic = {
			...
			"deliver_mode":order_ids_val.deliver_mode,
			"pickup_store":order_ids_val.pickup_store,
			"door_delivery_sale_point":order_ids_val.door_delivery_sale_point,
			"company":order_ids_val.company
		}
